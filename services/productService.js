...
async function getSitemapRoutes () {
    ...
    const receipesList = recipes.data.map(recipe => `/recipes/${recipe.slug}`)
    const blogList = blogs.data.map(blog => `/blog/${blog.slug}`)

    return [...productList, ...categoryList, ...receipesList, ...blogList]
}
